...
fn build_image_path(output_path: impl AsRef<Path>, blur: i32, darken: u8) -> Result<String> {
    let mut save_path = output_path.as_ref().to_owned();
    save_path.push(format!("blur_{}_darken_{:02}", blur, darken));
...
pub(crate) fn generate_image_grid(
    ...
    let first_image_path = build_image_path(&input_path, min_blur_radius, min_darken_number)?;
    ...
    for blur_index in 0..blur_number {
        ...
        for darken_index in 0..darken_number {
            ...
            let fetch_path = build_image_path(&input_path, blur_radius, darken)?;
